...
if value_select != "Выбрать":
    ...
    Start_datetime = st.text_input("Введите начальный момент времени в ВР")
    End_datetime = st.text_input("Введите конечный момент времени в ВР")
    # Periods_in_datetime = df_selected.shape[0]-1
    Periods_in_datetime = 47936
    Daterange = pd.date_range(Start_datetime, End_datetime, periods=Periods_in_datetime)
    st.write(Daterange)
    df_selected.index = Daterange
    st.write(df_selected)

    start_point, end_point = st.sidebar.slider('Выберите диапазон данных', 0, df_selected.shape[0]-1, (0, df_selected.shape[0]-1))
    ...
    df_select_interval = df_selected.loc[(start_point+m_a_step):end_point, 'Averaged']

    # dframe.write()
    # dframe.write(df_selected)
    linechart = st.line_chart(df_select_interval)
    st.dataframe(df_select_interval)
    # st.write(df_selected)
    ...
    if stationar_test:
        dfuller_res = adfuller(df_select_interval)[1]
        st.sidebar.write("Результаты теста на стационарность (p-value): ", dfuller_res)
        if dfuller_res < 5*10**(-2):
            st.sidebar.write("Ряд стационарен!")
            st.sidebar.write("[по критерию p < 0.05]")
        else:
            st.sidebar.write("Ряд не стационарен!")
    
    null_check = st.sidebar.button("Проверка на пропуски")
    if null_check:
        null_res = df_select_interval.isnull().values.any()
        if null_res == True:
            st.sidebar.write("В ряде присутствуют пропуски данных")
        else:
            st.sidebar.write("Пропусков нет")
    ...
    if operation_select == "Анализ":
        ...
        if method_select == "Вейвлет преобразование":
            ...
            if wavelet_select != "Выбрать":
                ...
                coef, freqs = pywt.cwt(df_select_interval, np.arange(1, T_s_len/4), mother_switcher.get(wavelet_select))
                ...
                # I = np.empty((len(freqs)))
                # for j in range(len(freqs)-1):
                #     for i in range(len(df_select_interval)-1):
