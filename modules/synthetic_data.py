...
def generate_synthetic_time_series(
    ...
    S_1_coef: float = 0.0,
    S_1_period: float = 0.0,
    S_2_coef: float = 0.0,
    S_2_period: float = 0.0,
    S_3_coef: float = 0.0,
    S_3_period: float = 0.0,
    C_1_coef: float = 0.0,
    C_1_period: float = 0.0,
    C_2_coef: float = 0.0,
    C_2_period: float = 0.0,
    C_3_coef: float = 0.0,
    C_3_period: float = 0.0,
    NoiseCoef: float = 0.0,
    ...
    if S_1_period != 0:
        sinusoids += S_1_coef * np.sin(2 * np.pi * t / S_1_period)
    if S_2_period != 0:
        sinusoids += S_2_coef * np.sin(2 * np.pi * t / S_2_period)
    if S_3_period != 0:
        sinusoids += S_3_coef * np.sin(2 * np.pi * t / S_3_period)

    cosines = np.zeros(length)
    if C_1_period != 0:
        cosines += C_1_coef * np.cos(2 * np.pi * t / C_1_period)
    if C_2_period != 0:
        cosines += C_2_coef * np.cos(2 * np.pi * t / C_2_period)
    if C_3_period != 0:
        cosines += C_3_coef * np.cos(2 * np.pi * t / C_3_period)

    trend = TrendSlope * t if TrendSlope != 0 else np.zeros(length)
    noise = (
        np.random.normal(0, NoiseCoef, length) if NoiseCoef != 0 else np.zeros(length)
    )
