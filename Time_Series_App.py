...
# Преобразуем столбец Время в индекс
time_series_selected.set_index(date_column_select, inplace=True)

# datetime_column = time_series_selected[date_column_select]
...
# st.write(type(time_series_selected_limited))
...
if MA_checkbox == False:
    ...
    MA_step = int(st.sidebar.number_input(
        "Введите шаг скользящего среднего", min_value=1, max_value=T_s_len
    ))


    st.line_chart(time_series_selected_limited)
    numeric_columns = list(time_series_selected_limited.select_dtypes(include=["int", "float"]).columns)
    # st.write(time_series_selected_limited[numeric_columns])
    ...
    time_series_selected_limited["Сглаженный"] = time_series_selected_limited[numeric_columns].rolling(
    ...
    with col1:
        ...
        dframe_chart_MA = st.line_chart(
            time_series_selected_limited.iloc[MA_step:][["Сглаженный"]]
        )
    with col2:
        "Исходный ряд"
        st.line_chart(time_series_selected_limited.iloc[:][value_select])
