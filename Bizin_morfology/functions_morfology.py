    ...
    for col_ind in range(len(form_parameters) - 1):
        if (col_ind % 2 == 0):
            if form_parameters[col_ind] == 0:
                ...
                m = [-1, 1]
                tmp = np.hstack([np.zeros(form_parameters[col_ind] - 1), m, np.zeros(size - form_parameters[col_ind] - 1)])
                A = np.vstack([A, tmp])
                tmp = np.hstack([np.zeros(form_parameters[col_ind]), m, np.zeros(size - form_parameters[col_ind] - 2)])
                A = np.vstack([A, tmp])
