...
def Convex_numeric(time_series, period, delta):
    ...
    for i in range(1, len(form_parameters) - 1):
        ...
        for infl_p in range(
            form_parameters[i - 1] + period // 2 - delta,
            form_parameters[i - 1] + period // 2 + delta + 1,
        ):
            infl_p = min(infl_p, size)
            inner_done = infl_p - (form_parameters[i - 1] + period // 2 - delta)
            print(
                round(
                    (outer_done * inner_cycle + inner_done)
                    / (outer_cycle * inner_cycle)
                    * 100,
                    2,
                ),
                "%",
    ...
    periods = np.array(
        [(periods_1[i] + periods_1[i + 1]) / 2 for i in range(0, len(periods_1) - 1, 2)]
    )
    ...
    ax1.plot(
        x, APPROXIMATION, "b", linewidth=1, label="result of morphological filtering"
    )
    ax1.plot(
...
t_s_period = st.sidebar.slider(
    "Выберите ожидаемую периодичность ряда",
    min_value=1,
    max_value=len(time_series) // 2,
)
...
st.subheader("Морфологический анализ")
